...
cd commandsmjo
chmod +x install.sh
./install.sh           # zsh 버전 (기본)
# 또는
./install.sh --bash    # bash 버전 (zsh 안 쓰는 경우)
```

> 💡 **bash 버전?**
> v2.3부터 `menu.bash` (bash 4+ 호환) 가 함께 제공됩니다. zsh를 따로 설치하기 싫거나 서버 기본 셸이 bash인 환경에서 그대로 쓰고 싶을 때 `--bash` 플래그를 사용하세요. 설치 위치는 `~/.bash_menu/`, 등록 파일은 `~/.bashrc` (없으면 `~/.bash_profile`) 입니다.
