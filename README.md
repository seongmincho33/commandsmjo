...
cd commandsmjo
chmod +x install.sh
./install.sh             # 현재 셸($SHELL)을 자동 감지 — bash면 bash, zsh면 zsh
# 강제 옵션도 지원
./install.sh --bash      # bash 버전 강제
./install.sh --zsh       # zsh 버전 강제
```

> 💡 **bash 버전?**
> v2.3부터 `menu.bash` (bash 4+ 호환) 가 함께 제공되며, 설치 스크립트가 사용자의 로그인 셸을 자동으로 감지해 알맞은 버전을 깔아줍니다. 서버 기본 셸이 bash인 환경에서도 zsh를 따로 설치할 필요가 없습니다. bash 버전 설치 위치는 `~/.bash_menu/`, 등록 파일은 `~/.bashrc` (없으면 `~/.bash_profile`) 입니다.
...
**옵션 A — bash 버전을 그대로 설치 (zsh 설치 불필요, 권장)**

```bash
./install.sh --bash
# 새 터미널 또는: source ~/.bashrc
dxk
```

→ 별도의 zsh 설치 없이 bash에서 `dxk`가 바로 동작합니다.

**옵션 B — zsh를 깔고 zsh 버전을 쓰기**

zsh 버전을 이미 설치했는데 `dxk`가 안 된다면, 지금 셸이 bash라서 그래요:
...
> ✅ `~/.zsh_menu/` · `~/.bash_menu/` 디렉토리와 `~/.zshrc` · `~/.bashrc` · `~/.bash_profile` 의 등록 라인이 깔끔하게 제거됩니다 (양쪽 버전 모두).
> ❌ zsh / bash 자체는 삭제되지 않으니 안심하세요.
...
```
zsh_seongmin/
├── menu.zsh             # 🧠 메인 메뉴 — zsh 버전
├── menu.bash            # 🧠 메인 메뉴 — bash 4+ 버전 (v2.3+)
├── install.sh           # 🍎🐧 macOS/Linux 설치 스크립트 (--bash / --zsh / --uninstall)
